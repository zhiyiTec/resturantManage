...
//用于跳转页面
function toPage_RM(pn) {
    cuPn_RM = pn;
    $.ajax({
        url: host + "/getAllReturnInfo",
        data: {
            pn: pn
        },
        dataType: "json",
        ...
        success: function(data) {
            ...
            showEmpMesage_RM(emp);
            //获取成功之后要做的第二件事情就是：解析并显示分页信息
            showPageInfo_RM(data);
...
//解析显示学生信息
function showEmpMesage_RM(data) {
    //每一次构建之前先把数据清空
    $("#tbody_data").empty();
    ...
    $.each(data, function(index, item) {
        ...
        $("#tr" + index).append("<td>" + item.returnTime + "</td>");
        $("#tr" + index).append("<td>" + item.medinceName + "</td>");
        $("#tr" + index).append("<td>" + item.returnName + "</td>");
        $("#tr" + index).append("<td>" + item.connector + "</td>");
        $("#tr" + index).append("<td id='bu" + index + "'></td>");

        $("#bu" + index).append(
            "<button class='btn btn-danger btn-sm delete_RM' data-delete='" +
            item.returnId +
            "' data-name='" +
            item.returnId +
            "'>删除</button>"
...
function showPageNav_RM(data) {
    ...
    if (data.results.pageNum > 1) {
        var dataPN = 1;
        $(".pagination").append(
            "<li><a href='#' data-pn='" + dataPN + "' class='pn_RM'>首页</a></li>"
        );
    }
    //上一页的值
    ...
    if (data.results.hasPreviousPage) {
        $(".pagination").append(
            ...
            prevoiuePage +
            "' class='pn_RM'> <span aria-hidden='true' class='span-style'>&laquo;</span></a></li>"
        );
    }
    $.each(data.results.navigatepageNums, function(index, item) {
        if (item == data.results.pageNum) {
            $(".pagination").append(
                "<li class='active pn_RM'><a href='#' class='pn_RM' data-pn='" +
                item +
                "'>" +
                item +
                "</a></li>"
    ...
    if (data.results.hasNextPage) {
        ...
        $(".pagination").append(
            ...
            nextPage +
            "' class='pn_RM'> <span aria-hidden='true' class='span-style'>&raquo;</span></a></li>"
        );
        //末页的值
        ...
        $(".pagination").append(
            "<li ><a href='#' class='pn_RM' data-pn='" + lastPage + "'>末页</a></li>"
        );
    }
}
//用于设置翻页的点击事件
$(document).on("click", ".pn_RM", function() {
    var pageNumber = $(this).attr("data-pn");
    cuPn_RM = pageNumber;
    toPage_RM(pageNumber);
});
//用于给动态产生的删除按钮添加点击事件
$(document).on("click", ".delete_RM", function() {
    var emp_id = $(this).attr("data-delete");
    var emp_Name = $(this).attr("data-name");
    var re = confirm("确认要删除【" + emp_Name + "】这条退货信息");
    if (!re) {
        return false;
    }
    $.ajax({
        url: host + "/deleteReturn",
        data: {
            pI: emp_id
        },
        dataType: "json",
...
//实现删除多个员工的按钮功能
$(document).on("click", "#empDeleBtn_RM", function() {
...
//按药品名查询
$(document).on("click", "#searchbyname_RM", function() {
    var name = $("#searchInput").val();

    $.ajax({
        url: host + "/searchReturnByIDs",
        data: {
            pn: 1,
            name: name
        },
