...
$(document).on("click", "#sailManage", function() {
    ...
    $("#nav").append(
        " <ol class='breadcrumb p-l-15 '> <li><a href='#' class='' disabled='disabled'>Home</a></li> <li><a href='#'>药店销售管理</a></li> <li class='active'>销售管理</li></ol>"
    );
    ...
    $("#cotent").append(
        "<table class='table table-striped table-bordered table-hover'> <thead><tr> <th><input type='checkbox' id='check_ALl' /></th><th>日期</th><th>客户</th><th>药品编码</th><th>药品名称</th><th>药品类别</th><th>数量</th><th>总价</th><th>操作</th></tr> </thead><tbody id='tbody_data'></tbody></table>"
    );
    //    < !--用于显示分页信息 -->
    $("#right").append(
...
function showEmpMesage_SM(data) {
    ...
    $.each(data, function(index, item) {
        ...
        $("#tr" + index).append("<td>" + item.buyTime + "</td>");
        ...
        $("#tr" + index).append("<td>" + item.count + "</td>");
...
$(document).on("click", ".delete_SM", function() {
    ...
    var re = confirm("确认要删除【" + emp_Name + "】这条销售信息");
    if (!re) {
        return false;
    }
    $.ajax({
        url: host + "/deleteSailInfo_SM",
        data: {
            sI: emp_id
        },
        dataType: "json",
