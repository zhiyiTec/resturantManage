...
function showEmpMesage_msm(data) {
    ...
    $.each(data, function(index, item) {
        ...
        $("#bu" + index).append("  ");
        $("#bu" + index).append(
            "<button class='btn  btn-warning btn-sm recharge_msm' data-delete='" +
            item.userId +
            "' data-name='" +
            item.buyerName +
            "' >会员充值</button>"
        ...
        $("#bu" + index).append("  ");
        $("#bu" + index).append(
            "<button class='btn  btn-default btn-sm spend_bu' data-delete='" +
            item.userId +
            "' data-name='" +
            item.buyerName +
            "' >消费</button>"
...
$(document).on("click", "#AddM_bI", function() {
    ...
    if (empty(cout)) $('#cout_bI').popover('show')
    else {
        $.ajax({
            url: host + "/updateUser",
            data: {
...
//实现会员充值的功能
$(document).on("click", ".recharge_msm", function() {
    var userid = $(this).attr("data-delete");
    var username = $(this).attr("data-name");
    $("#recharge_name").val(username);
    $("#recharge_phone").val(userid);
    $("#recharge_add").modal({
        backdrop: "static"
    });
});
//实现会员充值的功能
$(document).on("click", "#recharge_add_update", function() {
    var userid = $("#recharge_phone").val();
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            var re = data.re;
            if (re == 0) alert("添加充值记录失败");
            else if (re == 1) alert("添加充值信息失败");
            else if (re == 2) {
                $("#recharge_money").val("");
                alert("充值成功");
                $("#recharge_add").modal("hide");
            }
...
//实现会员的消费功能
$(document).on("click", ".spend_bu", function() {
    var userid = $(this).attr("data-delete");
    var name = $(this).attr("data-name");
    $.ajax({
        url: host + "/getCountRest",
        data: {
            userid: userid,
            name: name
        },
        ...
        crossDomain: true,
        success: function(data) {
            if (data.re == null) { alert("未查询到您的账户里有充值信息，请先充值"); } else {
                $("#spend_name").val(name)
                $("#spend_phone").val(userid)

                $("#spend_rest_money").val(data.re.restmoney);
...
$(document).on("click", "#spend_count_modal", function(e) {

    var id = $("#spend_phone").val();
    var cumoney = $("#spend_rest_money").val()
    var cumoney_int = parseInt(cumoney, 10);
    var spendMoeny = $("#spendmoney").val();
    var spendMoeny_int = parseInt(spendMoeny, 10);
    var re = spendMoeny_int - cumoney_int;

    if (re > 0) $('#spendmoney').popover("show")
    else {
        ...
        $.ajax({
            ...
            crossDomain: true,
            success: function(data) {
                if (data.re == 0) alert("支付失败");
                else {
                    $("#spen_modal").modal("hide");
                    $("#spendmoney").val("");
