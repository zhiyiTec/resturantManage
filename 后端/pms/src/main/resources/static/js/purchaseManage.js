...
$(document).on("click", "#PurchaseManage", function() {
    if (vertifacation_token(logininfo) != 0) {
    ...
    $("#cotent").append(
        "<table class='table table-striped table-bordered table-hover'> <thead><tr> <th><input type='checkbox' id='check_ALl' /></th><th>用户名</th><th>电话</th><th>操作</th></tr> <thead><tbody id='tbody_data'></tbody></table>"
    );
    //    < !--用于显示分页信息 -->
    $("#right").append(
...
function showEmpMesage_PM(data) {
    ...
    $.each(data, function(index, item) {
        ...
        $("#bu" + index).append(
            "<button class='btn btn-danger btn-sm showInfoSpend' data-delete='" +
            item.userid +
            "' data-name='" +
            item.name +
            "'>显示详细信息</button>"
...
//用于设置“显示详细信息事件”
$(document).on("click", ".showInfoSpend", function() {
    var userid = $(this).attr("data-delete");

    var url = host + "/getSpendDetailById";
    var data = {
        userid: userid
    };
    var transmission_type = "post";
    var re = ajax_get(url, data, transmission_type);
    $("#tbody_data_morespenddetail").empty();
    $.each(re.re, function(index, item) {


        $("#tbody_data_morespenddetail").append("<tr id='tr_re" + index + "'></tr>");
        $("#tr_re" + index).append("<td>" + item.spendtime + "</td>");
        $("#tr_re" + index).append("<td>" + item.spendnumber + "元</td>");

    });
    $("#morespenddetail").modal({
        backdrop: "static"
    });
